...
using Wpf.BLPmodel.Pages.MenuPage.ViewModels;
using Wpf.BLPmodel.Pages.MenuPage.Views;
...
namespace Wpf.BLPmodel.NavigationModule.Helpers {
    public static class InitializeViewHelper {
        ...
        public static void Run(PageNames pageName, IUnityContainer unityContainer, IRegionManager regionManager) {
            switch(pageName) {
                case PageNames.MenuView:
                    regionManager.Regions["RegionShell"].Add(GetMenuView(unityContainer));
                    return;
        ...
        private static UserControl GetMenuView(IUnityContainer unityContainer) {
            var view = unityContainer.Resolve<MenuView>();           
            view.DataContext = unityContainer.Resolve<MenuViewModel>();
            return view;           
        }
